...
2. 包内独立 `COMMIT-INFO` 是否存在，并逐文件将源码与 `git show e1e06bd:path` 比对；
3. `global.json` 是否固定 `.NET SDK 10.0.301` 且 `rollForward=disable`；
4. 测试源码中的断言是否真实存在，不能只根据日志名称判断；
5. `134/134` 历史日志与 `136/136` 当前目标提交结果是否有时间线和重新运行证据支持；
...
若发现历史日志中的目标提交或测试计数与当前包声明不一致，请引用 `EVIDENCE-CORRECTION`，并将未被重新运行直接支持的字段标为 `UNKNOWN` 或 `NOT_EXECUTED`，不得用文档文字覆盖冲突。
